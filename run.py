...
def user():
    ...
    while validation:
        try:
            ...
            print("Hello we are processing the data you provided...\n")
...
def new_user():
    ...
    while True:
        try:
            ...
            print(f"You have entered: '{n_user}'. This is not a" +
...
def get_data(action, user):
    ...
    hrs = events_sheet.col_values(4)
    sbs = events_sheet.col_values(5)
    pss = events_sheet.col_values(6)
    locs = events_sheet.col_values(7)
    eventCounter = 0
    if action == 1:
        print("We are searching for your events...\n")
        ...
        print("You have scheduled the following events:\n")
        for id, user_id, day, hr, sb, ps, loc\
                in zip(ids, user_ids, days, hrs, sbs, pss, locs):
            if user_id == user[0]:
                print(f"#{id}. {day} Meeting at {hr}" +
            ...
            while True:
                try:
                    while True:
                        ...
                        break
                    if day_input == currentday and t1 > t2:
                        break
                    elif day_input < currentday:
                        break
                    else:
                        raise ValueError(
